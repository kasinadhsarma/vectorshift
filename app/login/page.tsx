...
import { TwoFactorVerification } from "@/app/components/auth/two-factor-auth"
...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (returnUrl) {
      // Store returnUrl in state or ref if needed
  ...
  const completeLogin = (token: string, user: any) => {
    ...
    toast({
      ...
      description: "Welcome back to VectorShift!",
    })
  ...
  const handleTwoFactorVerified = (token: string) => {
    // Get user info from local storage (email only)
    const user = { email }
